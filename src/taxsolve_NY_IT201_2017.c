/************************************************************************/
/* TaxSolve_NY_IT-201_2017.c - NY State Tax form IT-201 for 2017.	*/
/* Copyright (C) 2003-2017 - Aston Roberts, Skeet Monker		*/
/* 									*/
/* Compile:   gcc taxsolve_NY_IT201_2017.c -o taxsolve_NY_IT201_2017	*/
/* Run:	      ./taxsolve_NY_IT201_2017  NY_IT201_2017.txt 		*/
/* 									*/
...
/* Modified for NY 2005-2017 taxes - Skeet Monker			*/
/************************************************************************/
...
double TaxRateFunction( double income, int status )
{
 ...
 switch (status)
  {
   case MARRIED_FILLING_JOINTLY: case WIDOW:						/* NOT Updated for 2017. */
	if (income <=   17050.0) tax = 	         0.04 * income; else
	if (income <=   23450.0) tax =   682.0 + 0.045  * (income - 17050.0); else
	if (income <=   27750.0) tax =   970.0 + 0.0525 * (income - 23450.0); else
	if (income <=   42750.0) tax =  1196.0 + 0.059  * (income - 27750.0); else
	if (income <=  160500.0) tax =  2088.0 + 0.0645 * (income - 42750.0); else
	if (income <=  321050.0) tax =  9676.0 + 0.0665 * (income - 160500.0); else
	if (income <= 2140900.0) tax = 20352.0 + 0.0685 * (income - 321050.0); 
	else			tax = 145012.0 + 0.0882 * (income - 2140900.0);
      break;
   case SINGLE: case MARRIED_FILLING_SEPARAT:
	if (income <=    8450.0) tax =  	  0.04   * income; else
	if (income <=   11650.0) tax =    338.0 + 0.045  * (income - 8450.0); else
	if (income <=   13850.0) tax =    482.0 + 0.0525 * (income - 11650.0); else
	if (income <=   21300.0) tax =    598.0 + 0.059  * (income - 13850.0); else
	if (income <=   80150.0) tax =   1037.0 + 0.0645 * (income - 21300.0); else
	if (income <=  214000.0) tax =   4833.0 + 0.0665 * (income - 80150.0); else
	if (income <= 1070350.0) tax =  13734.0 + 0.0685 * (income - 214000.0); 
	else 		 	 tax =  72394.0 + 0.0882 * (income - 1070350.0);
      break;
   case HEAD_OF_HOUSEHOLD:
	if (income <=   12750.0) tax = 	         0.04 * income; else
	if (income <=   17550.0) tax =   510.0 + 0.045  * (income - 12750.0); else
	if (income <=   20800.0) tax =   726.0 + 0.0525 * (income - 17450.0); else
	if (income <=   32000.0) tax =   897.0 + 0.059  * (income - 20800.0); else
	if (income <=  106950.0) tax =  1557.0 + 0.0645 * (income - 32000.0); else
	if (income <=  267500.0) tax =  6392.0 + 0.0665 * (income - 106950.0); else
	if (income <= 1605650.0) tax = 17068.0 + 0.0685 * (income - 267500.0);
	else			tax = 108732.0 + 0.0882 * (income - 1605650.0);
      break;
...
void Report_bracket_info( double income, double tx, int status )
{
 ...
 switch (status)
  {
   case MARRIED_FILLING_JOINTLY: case WIDOW:						/* NOT Updated for 2017. */
	if (income <=   17050.0) rate = 0.04;  else
	if (income <=   23450.0) rate = 0.045;  else
	if (income <=   27750.0) rate = 0.0525;  else
	if (income <=   42750.0) rate = 0.059;  else
	if (income <=  160500.0) rate = 0.0645;  else
	if (income <=  321050.0) rate = 0.0665;  else
	if (income <= 2140900.0) rate = 0.0685;  else  rate = 0.0882;
      break;
   case SINGLE: case MARRIED_FILLING_SEPARAT:
	if (income <=    8450.0) rate = 0.04;  else
	if (income <=   11650.0) rate = 0.045;  else
	if (income <=   13850.0) rate = 0.0525;  else
	if (income <=   21300.0) rate = 0.059;  else
	if (income <=   80150.0) rate = 0.0645;  else
	if (income <=  214000.0) rate = 0.0665;  else
	if (income <= 1070350.0) rate = 0.0685;  else  rate = 0.0882;
      break;
   case HEAD_OF_HOUSEHOLD:
	if (income <=   12750.0) rate = 0.04;  else
	if (income <=   17550.0) rate = 0.045;  else
	if (income <=   20800.0) rate = 0.0525;  else
	if (income <=   32000.0) rate = 0.059;  else
	if (income <=  106950.0) rate = 0.0645;  else
	if (income <=  267500.0) rate = 0.0665;  else
	if (income <= 1605650.0) rate = 0.0685;  else  rate = 0.0882;
      break;
   default: printf("Error: Unhandled status\n"); exit(0); break;
  }
 ...
 if ((status==MARRIED_FILLING_JOINTLY) || (status==WIDOW))
  {
   ...
   if (income < 500000.0) tax = (income - 90000.00) * 0.03648 + 3071.0; else
			  tax = (income - 500000.0) * 0.03876 + 19155.00;
  }
 else
 if ((status==SINGLE) || (status==MARRIED_FILLING_SEPARAT))
  {
   ...
   if (income < 500000.0) tax = (income - 50000.00) * 0.03648 + 1706.0; else
			  tax = (income - 500000.0) * 0.03876 + 19255.00;
  }
 else
 if (status==HEAD_OF_HOUSEHOLD) 
  {
   ...
   if (income < 500000.0) tax = (income - 60000.00) * 0.03648 + 2047.0; else
			  tax = (income - 500000.0) * 0.03876 + 19230.00;
 }
...
void worksheet1()	/*Tax Computation Worksheet 1 (pg 56) */		/* NOT Updated for 2017. */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0645 * ws[2];
...
void worksheet2()	/*Tax Computation Worksheet 2 (pg 56) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0665 * ws[2];
...
void worksheet3()	/*Tax Computation Worksheet 3 (pg 56) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0685 * ws[2];
...
void worksheet4()	/*Tax Computation Worksheet 4 (pg 56) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0882 * ws[2];
...
void worksheet5()	/*Tax Computation Worksheet 5 (pg 57) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0665 * ws[2];
...
void worksheet6()	/*Tax Computation Worksheet 6 (pg 57) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0685 * ws[2];
...
void worksheet7()	/*Tax Computation Worksheet 7 (pg 57) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0882 * ws[2];
...
void worksheet8()	/*Tax Computation Worksheet 8 (pg 58) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0665 * ws[2];
...
void worksheet9()	/*Tax Computation Worksheet 9 (pg 58) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0685 * ws[2];
...
void worksheet10()	/*Tax Computation Worksheet 10 (pg 58) */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0882 * ws[2];
...
{ /* Worksheets from pages 56-58. Come here when AGI L[33] > $106,950. */
 switch (status)								/* NOT Updated for 2017. */
  {
	...
	if (L[33] <= 2140900.0)
	 {
	   ...
	   if ((L[33] > 160500.0) && (L[38] <= 321050.0))
	    worksheet2();
	   else
	   if ((L[33] > 321050.0) && (L[38] > 321050.0))
	    worksheet3();
	   else
	    worksheet4();
	 }
 ...
 switch (status)	/* Determine the Std. Deduction. Pg. 20. */
  {
   case SINGLE: if (Dependent)   std_ded = 3100.0; 
		else 		 std_ded = 7950.0;			/* NOT Updated for 2017. */
	break;
   case MARRIED_FILLING_JOINTLY: std_ded = 15950.0; break;
   case MARRIED_FILLING_SEPARAT: std_ded =  7960.0; break;
   case HEAD_OF_HOUSEHOLD: 	 std_ded = 11150.0; break;
   case WIDOW: 			 std_ded = 15950.0; break;
  }
