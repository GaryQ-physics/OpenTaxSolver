/************************************************************************/
/* TaxSolve_usa_fed1040_2017.c - OpenTaxSolver for USFed1040 		*/
/* Copyright (C) 2017 - Aston Roberts					*/
/* 									*/
/* Tax Solver for US Fedral 1040 Income Tax return for 2017 Tax Year.	*/
/* 									*/
/* OTS Project Home Page and Updates:  					*/
/*		http://opentaxsolver.sourceforge.com/			*/
/* 									*/
/* Compile:   cc taxsolve_US_1040_2017.c -o taxsolve_US_1040_2017       */
/* Run:       ./taxsolve_US_1040_2017  Fed1040_2017.txt                 */
/* 									*/
...
double  L60b=0.0, collectibles_gains=0.0, ws_sched_D[MAX_LINES], idws_thresh=0.0, idws[MAX_LINES];
				...
				/* Following values taken from 1040 Instructions. */	/* Updated for 2017. */
double brkpt[4][9]={
		  { 0.0,   9325.0,  37950.0,  91900.0, 191650.0, 416700.0, 418400.0, 9e9 }, /* Single */
		  { 0.0,  18650.0,  75900.0, 153100.0, 233350.0, 416700.0, 470700.0, 9e9 }, /* Married, filing jointly. */
		  { 0.0,   9325.0,  37950.0,  76550.0, 116675.0, 208350.0, 235350.0, 9e9 }, /* Married, filing separate. */
		  { 0.0,  13350.0,  50800.0, 131200.0, 212500.0, 416700.0, 444550.0, 9e9 }  /* Head of Household. */
		     };
  double txrt[4][9] ={
...
void Report_bracket_info( double x, double addedtx, int status )  
{
  ...
  printf(" You are in the %2.1f%% marginal tax bracket,\n and you are paying an effective %2.1f%% tax on your income.\n",
          100.0 * txrt[status][bracket], 100.0 * (tx + addedtx) / (x + 1e-9) );
...
/*-----------------------------------------------------------------*/
void capgains_qualdividends_worksheets( int status, double L9b )		/* Updated for 2017. */
{
 double ws[50];
 int j;
 ...
 switch (status)
  {
   case SINGLE: case MARRIED_FILLING_SEPARAT: ws[8] = 37950.0; break;
   case MARRIED_FILLING_JOINTLY: case WIDOW:  ws[8] = 75900.0; break;
   case HEAD_OF_HOUSEHOLD: 		      ws[8] = 50800.0; break;
  }
 ...
 switch (status)
  {
   case SINGLE:  			      ws[15] = 418400.0;  break;
   case MARRIED_FILLING_SEPARAT:	      ws[15] = 235350.0;  break;
   case MARRIED_FILLING_JOINTLY: case WIDOW:  ws[15] = 470700.0;  break;
   case HEAD_OF_HOUSEHOLD: 		      ws[15] = 444550.0;  break;
  }
 ...
 if (status == MARRIED_FILLING_SEPARAT)
  {
   ...
   if (amtws[28] > 249450.0) amtws[28] = amtws[28] + 0.25 * (amtws[28] - 247450.0);
  }
  ...
  { /*Lines31-33*/
    ...
    if ((L[13] == 0.0) && (L9b == 0.0) && ((SchedD[15] <= 0.0) || (SchedD[16] <= 0.0)))
     ...
     { /*Part-III*/ /* Part-III must be done here, due to dependency on line 31. */
       ...
       switch (status)
	{
	   case SINGLE:  			      amtws[49] = 418400.0;  break;
	   case MARRIED_FILLING_SEPARAT:	      amtws[49] = 235350.0;  break;
	   case MARRIED_FILLING_JOINTLY: case WIDOW:  amtws[49] = 470700.0;  break;
	   case HEAD_OF_HOUSEHOLD: 		      amtws[49] = 444550.0;  break;
	   default:  printf("Status %d not handled.\n", status);  exit(1); 
	}
       amtws[50] = amtws[45];
...
void CapitalLossCarryOverWorksheet( char *fedlogfile, struct FedReturnData *LastYearsReturn )	/* Updated for 2017. */
...
/*									*/
/************************************************************************/
void get_cap_gains( char *emssg	)							/* Updated for 2017. */
{
 ...
 if (short_trades)
  {
   print_capgain_list( short_trades, 1, "Form 8949 Part-I, Short-Term Cap Gains+Losses, CHECK (A) Basis Reported to IRS:", "9 9\n F8949_ckA X" );
 ...
 if (long_trades)
  {
   print_capgain_list( long_trades, 3, "Form 8949 Part-II, Long-Term Cap Gains+Losses, CHECK (D) Basis Reported to IRS:", "10 10\n F8949_ckD X" );
 ...
 if (short_trades)
  {
   print_capgain_list( short_trades, 1, "Form 8949 Part-I, Short-Term Cap Gains+Losses, CHECK (B) Basis NOT Reported to IRS:", "9 9\n F8949_ckB X" );
 ...
 if (long_trades)
  {
   print_capgain_list( long_trades, 3, "Form 8949 Part-II, Long-Term Cap Gains+Losses, CHECK (E) Basis NOT Reported to IRS:", "10 10\n F8949_ckE X"  );
...
/*------------------------------------------------------*/
void sched_D_tax_worksheet( int status, double L9b )			/* Updated for 2017. */
{
 double ws[100];
 int k;
 ...
 switch (status) 
  { case SINGLE: case MARRIED_FILLING_SEPARAT: ws[15] = 37950.0; break;
    case MARRIED_FILLING_JOINTLY: case WIDOW:  ws[15] = 75900.0; break;
    case HEAD_OF_HOUSEHOLD:      	       ws[15] = 50800.0; break;
  }
 ws[16] = smallerof( ws[1], ws[15] );
 ...
 if (ws[1] != ws[16])
  { /*lines21-41*/
   ...
   switch (status) 
    { case SINGLE: 			ws[24] = 418400.0;  break;
      case MARRIED_FILLING_SEPARAT: 	ws[24] = 235350.0;  break;
      case MARRIED_FILLING_JOINTLY: 
      case WIDOW:  			ws[24] = 470700.0;  break;
      case HEAD_OF_HOUSEHOLD:		ws[24] = 444550.0;  break;
    }
...
/*-------------------------------------------------------*/
void SocSec_Worksheet()							/* Updated for 2017. */
{
 double ws[100];
 int k;
 if (L[20] == 0.0) return;
 ...
 if (status == MARRIED_FILLING_JOINTLY)
  ws[10] = 12000.0;      						/* Updated for 2017. */
 else
  ws[10] = 9000.0;
 ...
 */
 ...
 get_parameter( infile, 's', word, "Under65?" );	/* Age at end of 2017. (Y/N) If Married/Joint, Y=both-under-65, N=both-over-65, 1=One-over-65 */
 ...
 GetLine( "L15a", &tmpval );	/* IRA distributions. */
 showline_wlabel( "L15a", tmpval );

 GetLine( "L15b", &L[15] );	/* Taxable IRA distributions (pg 24). */
 ShowLineNonZero(15);

 GetLine( "L16a", &tmpval );	/* Pensions and annuities. */
 ...
 GetLine( "L20a", &L[20] );	/* Social Security benefits. */
 ...
 switch (status)	/* Check for minimum income to file. */				/* Updated for 2017. */
  {
   case SINGLE:  		  if (under65) exemption_threshold = 10400.0;
				  else  exemption_threshold = 11950.0;
	break;
   case MARRIED_FILLING_JOINTLY:  if (under65==2) exemption_threshold = 20800.0;
				  else 
				  if (under65==1) exemption_threshold = 22050.0;  
				  else  exemption_threshold = 23300.0;
				  if (under65 != 2) over65 = 1;
	break;
   case MARRIED_FILLING_SEPARAT:  exemption_threshold = 4050.0;
	break;
   case HEAD_OF_HOUSEHOLD: 	  if (under65) exemption_threshold = 13400.0;  
				  else  exemption_threshold = 14950.0;
	break;
   case WIDOW:  		  if (under65) exemption_threshold = 16750.0;  
				  else  exemption_threshold = 18000.0;
  }
 ...
 { /* Student loan interest calculation pg 37. */
  ...
  ws[4] = ws[2] - ws[3];
  if (status == MARRIED_FILLING_JOINTLY) ws[5] = 135000.0; else ws[5] = 65000.0;	/* Updated for 2017. */
  if (ws[4] > ws[5])
   {
    ws[6] = ws[4] - ws[5];
 ...
 switch (status)
  {
     case SINGLE: 				idws_thresh = 261500.0;  break;	  /* Updated for 2017. */
     case MARRIED_FILLING_JOINTLY: case WIDOW:  idws_thresh = 313800.0;  break;	  /* Updated for 2017. */
     case MARRIED_FILLING_SEPARAT:   		idws_thresh = 156900.0;  break;	  /* Updated for 2017. */
     case HEAD_OF_HOUSEHOLD:  			idws_thresh = 287650.0;  break;	  /* Updated for 2017. */
     default:  printf("Status %d not handled.\n", status);  exit(1); 
  }
 ...
 if (L[39] == 0.0)
  {
   S_STD_DEDUC   =  6350.0;						/* Updated for 2017. */
   MFS_STD_DEDUC =  6350.0;
   MFJ_STD_DEDUC = 12700.0;
   HH_STD_DEDUC  =  9350.0;
  }
 else
  { /* Std. Deduction chart for People who were Born Before January 2, 1953, or Were Blind, pg 40. */
    switch ((int)L[39])		/* Does not handle if someone claims you or joint-spouse as dependent. */
     {
      case 1: 
	S_STD_DEDUC   =  7900.0;					/* Updated for 2017. */
	...
	S_STD_DEDUC   =  9450.0;	/* Cannot happen, but set to appease compiler. */
	HH_STD_DEDUC  = 12450.0;	/* .. */
	break;
      case 4: 
 ...
 if (L[38] <= idws_thresh)
  ...
  { /* Deduction for Exemptions Worksheet (pg 41) */
    double thresh;
 ...
 GetLine( "L65", &L[65] );	/* 2017 estimated payments + amnt applied from last year. */
 ShowLineNonZero(65);
 ...
 GetLine( "L66b", &tmpval );	/* Non-Taxable Combat Pay. */
 if (tmpval != 0.0)
